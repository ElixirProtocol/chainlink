...
func (st *fakeStreamsTrigger) emitEvent(ctx context.Context) {
	...
	for feed, lastPrice := range st.lastPrice {
		...
		fullReport := newReport(ctx, st.lggr, idBytes32, lastPrice, nowMs)
	...
	for wf, reg := range st.registrations {
		if reg.cfg.MaxFrequencyMs > 0 && newTs%int64(reg.cfg.MaxFrequencyMs) != 0 {
			continue
		}
...
func newReport(ctx context.Context, lggr logger.Logger, feedID [32]byte, price int64, timestamp int64) []byte {
