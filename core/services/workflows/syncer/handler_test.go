...
func Test_Handler(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		...
		artifactsStore := artifacts.NewStore(lggr, mockORM, nil, fetcher, clockwork.NewFakeClock(), workflowkey.Key{}, emitter)
		h := NewEventHandler(lggr, nil, nil, emitter, rl, artifactsStore)
	...
	t.Run("fails to get secrets url", func(t *testing.T) {
		...
		artifactsStore := artifacts.NewStore(lggr, mockORM, nil, nil, clockwork.NewFakeClock(), workflowkey.Key{}, emitter)
		h := NewEventHandler(lggr, nil, nil, emitter, rl, artifactsStore)
	...
	var tt = []testCase{
		{
			...
			engineFactoryFn: func(ctx context.Context, wfid string, owner string, name workflows.WorkflowNamer, config []byte) (services.Service, error) {
...
func Test_workflowDeletedHandler(t *testing.T) {
	t.Run("success deleting existing engine and spec", func(t *testing.T) {
		...
		require.NoError(t, err)
		artifactsStore := artifacts.NewStore(lggr, orm, moduleStore, fetcher, clockwork.NewFakeClock(), workflowkey.Key{}, emitter)
		h := NewEventHandler(lggr, store, registry, emitter, rl, artifactsStore, WithEngineRegistry(er))
...
func Test_workflowPausedActivatedUpdatedHandler(t *testing.T) {
	t.Run("success pausing activating and updating existing engine and spec", func(t *testing.T) {
		...
		artifactsStore := artifacts.NewStore(lggr, orm, nil, fetcher, clockwork.NewFakeClock(), workflowkey.Key{}, emitter)
		h := NewEventHandler(lggr, store, registry, emitter, rl, artifactsStore, WithEngineRegistry(er))
