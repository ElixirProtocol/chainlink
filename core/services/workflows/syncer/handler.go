...
type engineFactoryFn func(ctx context.Context, wfid string, owner string, name workflows.WorkflowNamer, config []byte) (services.Service, error)
...
type WorkflowArtifactsStore interface {
	...
	// Secrets methods
	GetSecrets(ctx context.Context, secretsURL string) ([]byte, error)
	...
	ForceUpdateSecrets(ctx context.Context, secretsURLHash []byte) (string, error)
}
...
func (h *eventHandler) workflowRegisteredEvent(
	...
	if payload.SecretsURL != "" {
		...
		secrets, err = h.workflowArtifactsStore.GetSecrets(ctx, secretsURL)
		if err != nil {
...
func (h *eventHandler) engineFactoryFn(ctx context.Context, workflowID string, owner string, name workflows.WorkflowNamer, config []byte) (services.Service, error) {
	moduleConfig := &host.ModuleConfig{Logger: h.lggr, Labeler: h.emitter}
	sdkSpec, err := host.GetWorkflowSpec(ctx, h.lggr, moduleConfig, workflowID, h.workflowArtifactsStore, config)
	if err != nil {
...
func (h *eventHandler) forceUpdateSecretsEvent(
	...
	return h.workflowArtifactsStore.ForceUpdateSecrets(ctx, payload.SecretsURLHash)
}
