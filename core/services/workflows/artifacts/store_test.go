...
func Test_Handler_SecretsFor(t *testing.T) {
	...
	s := NewStore(lggr, orm, nil, fetcher.Fetch, clockwork.NewFakeClock(), encryptionKey, custmsg.NewLabeler())
...
func Test_Handler_SecretsFor_RefreshLogic(t *testing.T) {
	...
	s := NewStore(lggr, orm, nil, fetcher.Fetch, clock, encryptionKey, custmsg.NewLabeler())
