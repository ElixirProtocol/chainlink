...
func (h *Store) FetchWorkflowArtifacts(ctx context.Context, workflowID, binaryURL, configURL string) ([]byte, []byte, error) {
	...
	if binary, err = h.fetchFn(ctx, binaryURL, safeUint32(h.limits.MaxBinarySize)); err != nil {
	...
	if configURL != "" {
		if config, err = h.fetchFn(ctx, configURL, safeUint32(h.limits.MaxConfigSize)); err != nil {
...
func (h *Store) GetSecrets(ctx context.Context, secretsURL string) ([]byte, error) {
	return h.fetchFn(ctx, secretsURL, safeUint32(h.limits.MaxSecretsSize))
}

func (h *Store) ForceUpdateSecrets(ctx context.Context, secretsURLHash []byte) (string, error) {
...
func (h *Store) SecretsFor(ctx context.Context, workflowOwner, hexWorkflowName, decodedWorkflowName, workflowID string) (map[string]string, error) {
	...
	if !ok || h.clock.Now().Sub(lastFetchedAt) > h.secretsFreshnessDuration {
		updatedSecrets, innerErr := h.refreshSecrets(ctx, secretsURLHash)
		if innerErr != nil {
...
func (h *Store) refreshSecrets(ctx context.Context, secretsURLHash string) (string, error) {
	decodedHash, err := hex.DecodeString(secretsURLHash)
	if err != nil {
		return "", fmt.Errorf("failed to decode secrets URL hash: %w", err)
	}

	updatedSecrets, err := h.ForceUpdateSecrets(ctx, decodedHash)

	if err != nil {
		return "", fmt.Errorf("failed to force update secrets: %w", err)
	}
