...
func (s *InMemoryStore) Add(ctx context.Context, state *WorkflowExecution) (WorkflowExecution, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	_, ok := s.idToState[state.ExecutionID]
	if ok {
		return WorkflowExecution{}, fmt.Errorf("execution ID %s already exists in store", state.ExecutionID)
	}
...
func (s *InMemoryStore) GetUnfinished(ctx context.Context, workflowID string, offset, limit int) ([]WorkflowExecution, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()

	states := []WorkflowExecution{}
	for _, s := range s.idToState {
		if s.Status == StatusStarted && s.WorkflowID == workflowID {
