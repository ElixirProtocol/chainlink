...
func Test_SecretsWorker(t *testing.T) {
	...
	artifactsStore := artifacts.NewStore(lggr, orm, nil, fetcherFn, clockwork.NewFakeClock(), workflowkey.Key{}, emitter)
	eh := syncer.NewEventHandler(lggr, nil, nil, emitter, rl, artifactsStore)
...
func Test_RegistrySyncer_WorkflowRegistered_InitiallyPaused(t *testing.T) {
	...
	artifactsStore := artifacts.NewStore(lggr, orm, nil, fetcherFn, clockwork.NewFakeClock(), workflowkey.Key{}, emitter)
	handler := syncer.NewEventHandler(lggr, nil, nil, emitter, rl, artifactsStore, syncer.WithEngineRegistry(er))
...
func (m *mockEngineFactory) new(ctx context.Context, wfid string, owner string, name workflows.WorkflowNamer, config []byte) (services.Service, error) {
